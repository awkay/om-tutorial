...
(defn read-local
  ...
  (case key
    ...
    :person/mate {:value (p/parse-join-with-reader read-local env key :limit 3)}
...
(comment
  (defn generic-read-local
    ...
    (case key
      :people/by-id {:value (p/parse-join-with-reader read-local (assoc env :db-path []) (:key ast) true)}
